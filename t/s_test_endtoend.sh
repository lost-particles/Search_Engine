...
cat "$T_FOLDER"/myData/u.txt > d/urls.txt

./engine.sh

if $DIFF <(sort d/visited.txt) <(sort "$T_FOLDER"/myData/v.txt) > /dev/null;
then
...
if DIFF_PERCENT=$DIFF_PERCENT t/gi-diff.js <(sort d/global-index.txt) <(sort "$T_FOLDER"/myData/i.txt) > /dev/null;
then
